...
def execute_training_loop(
    world_model, 
    data_iterator, 
    training_epochs=1,  # Changed to default to 1 epoch
    lr=1e-4, 
    compute_device=None
):
    ...
    # Training loop - running for just 1 epoch
    for epoch in tqdm(range(training_epochs), desc='Training Progress'):
        ...
        # Save checkpoint
        checkpoint_model(world_model.state_dict(), "continued", output_dir="continued_model")
        ...
        print(f"Epoch Complete Summary:")
...
if __name__ == "__main__":
    print("Initializing training from checkpoint for 1 epoch")
    ...
    print("Building world model and loading from checkpoint...")
    world_model = MainModel(training_mode=True)
    
    # Load weights from checkpoint
    checkpoint_path = "checkpoint/best_model.pth"
    try:
        # Set weights_only to False if your checkpoint contains optimizer state too
        world_model.load_state_dict(torch.load(checkpoint_path, map_location=compute_device))
        print(f"Successfully loaded weights from {checkpoint_path}")
        
        # Ensure repr_dim is set (for evaluation)
        if not hasattr(world_model, 'repr_dim'):
            world_model.repr_dim = 128
            print("Added repr_dim attribute for evaluation compatibility")
    ...
    # Execute training loop for just 1 epoch
    ...
    checkpoint_model(world_model.state_dict(), "final", output_dir="continued_training")
